...
import SubmitIcon from '@material-ui/icons/ArrowUpward';
...
export default function LokerEdit() {
    ...
    useEffect(() => {
        ...
        const findLoker = loker.find(i => i.LokerId === params.LokerId);
        if (findLoker) {
            setForm(form => ({
                ...findLoker,
            ...
            const contentBlock = htmlToDraft(findLoker.Deskripsi || '');
    ...
    const handleSubmit = async e => {
        ...
        if (Object.values(findErrors).some(message => message !== "")) {
            ...
            try {
                ...
                const response = await api.put(`/loker/${params.LokerId}`, {
                    ...form,
                    Deskripsi
                });

                const newLokerData = loker.map((item) => {

                    if (item.LokerId === params.LokerId) {
                        return response.data
                    }
                    return item;
                })
                setLoker(newLokerData);
    ...
    const requestForModeration = async (e) => {
        setSubmitting(true);
        try {
            const response = await api.post(`/submit-loker/${params.LokerId}`)
            const newLokerData = loker.map((item) => {
            ...
            setLoker(newLokerData);
            enqueueSnackbar("Loker berhasil di submit untuk di moderasi oleh admin!", { variant: "success" })
    ...
    return <>
        <Helmet>
            <title>Edit Loker - JagoReact Job Board</title>
        </Helmet>
        <Typography className={classes.title} variant="h4" component="h3" paragraph>Edit Loker</Typography>
        <Paper className={classes.content} elevation={3}>
            ...
            <form onSubmit={handleSubmit} noValidate>
                ...
                <Grid container spacing={2} justify="center" className={classes.buttons}>
                    ...
                    {form.Status === "DRAFT" && <Grid item xs={12} sm={6}>
                        <div className={classes.btnWrapper}>
                            <Button
                                color="secondary"
                                variant="contained"
                                size="large"
                                onClick={requestForModeration}
                                fullWidth
                                disabled={isSubmitting}
                            ><SubmitIcon className={classes.iconLeft} /> Submit Moderasi</Button>
                            {isSubmitting && <CircularProgress size={24} className={classes.buttonProgress} />}
                        </div>
                    </Grid>}

                    {form.Status === "MODERATION" && <Grid item xs={12} sm={6}>
                        <Typography color="error" variant="subtitle1">Loker sedang dalam moderasi!</Typography>
                    </Grid>}
